...
利用者は日本におり、画面はすべてサーバー側で描画したうえでデータベースを
読む。したがって、関数の実行地とデータベースの所在地が離れていると、
1 ページの表示ごとにその往復が重なる。

| 対象 | 設定 | 備考 |
|---|---|---|
| Vercel Functions | `hnd1`（東京） | `vercel.json` の `regions` で指定する |
| Supabase | Northeast Asia (Tokyo) | **プロジェクト作成時にしか選べない** |

Vercel の Hobby プランでは、単一のリージョンのみ選択できる。複数指定は
Pro 以上を要する。

Supabase のリージョンは後から変更できない。誤ったリージョンで作成した
場合、プロジェクトを作り直すほかない。データが少ないうちに気づくこと。
